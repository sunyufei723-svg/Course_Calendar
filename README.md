...
`example2.jpg` 是按单节格子显示课程的选课时间表；识别器会合并同一天相邻、同名且周次/地点一致的格子。截图本身没有教学周次，部分标题被界面截断，必须在 GUI 或 JSON 中补全并核对后才能导出可靠的课表。

`example.transcribed.json` 和 `example.transcribed.ics` 是按示例图人工转录的演示文件，**尚未由课表提供者确认**。正式导入前请再核对教师姓名、交替周次及教室。
